...
LinksFrontrunning.currentProvider.sendAsync = function() {
  return LinksFrontrunning.currentProvider.send.apply(LinksFrontrunning.currentProvider, arguments);
};
...
contract('LinksFrontrunning', function(accounts) {
  ...
  before(async () => {
    instance = await LinksFrontrunning.deployed()
  })
  ...
  describe('2) Claim fund value as user2.', () => {
    ...
    before(async () => {
        ...
        message = web3_1.utils.soliditySha3(
          ...
          {type: 'address', value: LinksFrontrunning.address} // contract address
        )
  ...
  describe('4) Frontrunning attack from badBoy stealing user2 funds.', () => {
    ...
    before(async () => {
        ...
        initialBalance = await web3_1.eth.getBalance(destination2)
        message = web3_1.utils.soliditySha3(
    ...
    it('should frontrun fund tx with same parameters.', async () => {
      try {
    ...
    it('should increment destination balance by value',async () => {
      const finalBalance = await web3_1.eth.getBalance(destination2)
      const balance = web3_1.utils.toBN(initialBalance).add(web3_1.utils.toBN(value))
      assert.equal(balance, finalBalance)
    })
  })
